...
static int cat_undo_step(undo_buffer *const ub, const int64_t line, const int64_t pos, const int64_t len)
{
    ...
    if (ub->last_save_step > ub->cur_step)
    {
        ub->last_save_step = -1;
    }
...
void end_undo_chain(buffer *const b)
{
    ...
    if (b->undo.cur_step && b->undo.steps[b->undo.cur_step - 1].pos < 0)
    {
        b->undo.steps[b->undo.cur_step - 1].pos = -(1 + b->undo.steps[b->undo.cur_step - 1].pos);
    }
}
...
int add_to_undo_stream(undo_buffer *const ub, const char *const p, const int64_t len)
{
    ...
    if (!ub->cur_step || ub->steps[ub->cur_step - 1].len < 0)
    {
        return -1;
    }
