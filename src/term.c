...
void set_attr(const uint32_t attr)
{
    ...
    if (attr & INVERSE)
    {
        OUTPUT1(ne_enter_reverse_mode);
    }
    if (attr & BOLD)
    {
        OUTPUT1(ne_enter_bold_mode);
    }
    if (attr & UNDERLINE)
    {
    ...
    if (attr & DIM)
    {
        OUTPUT1(ne_enter_dim_mode);
    }
    if (attr & BLINK)
    {
        OUTPUT1(ne_enter_blink_mode);
    }
...
static void set_scroll_region(const int start, const int stop)
{
    ...
    char *buf;
    if (ne_change_scroll_region)
    {
        buf = tparm(ne_change_scroll_region, start, stop);
    }
    else
    {
        buf = tparm(ne_set_window, start, stop, 0, ne_columns - 1);
    }
...
void move_cursor(const int row, const int col)
{
    ...
    if (!ne_move_standout_mode)
    {
        turn_off_standout();
    }
    if (!ne_move_insert_mode)
    {
        turn_off_insert();
    }
    cmgoto(row, col);
...
void clear_end_of_line(const int first_unused_hpos)
{
    ...
    else
    {
        ...
        turn_off_insert();
        for (int i = curX; i < first_unused_hpos; i++)
        {
            putchar(' ');
        }
...
void insert_chars(const char *start, const uint32_t *const attr, const int raw_len, const bool utf8)
{
    ...
    if (ne_parm_ich)
    {
        ...
        if (start != NULL)
        {
            if (utf8) for (int i = 0; i < raw_len; i += utf8len(start[i]))
                {
                    width += output_width(utf8char(start + i));
                }
            else for (int i = 0; i < raw_len; i++)
                {
                    width += output_width(start[i]);
                }
        }
        else
        {
        ...
        if (start)
        {
            output_chars(start, attr, raw_len, utf8);
        }
...
void delete_chars(int n)
{
    ...
    if (ne_parm_dch)
    {
        ...
        {
            OUTPUT1(ne_delete_character);
        }

    if (!delete_in_insert_mode)
    {
        OUTPUT_IF(ne_exit_delete_mode);
    }
}
...
int ins_del_lines(const int vpos, const int n)
{
    ...
    if (!ne_memory_below && vpos + i >= ne_lines)
    {
        return false;
    }
    ...
    if (scroll_region_ok)
    {
        ...
        {
            set_scroll_region(vpos, specified_window - 1);
        }
        ...
        if (n < 0)
        {
            ...
            while (i-- != 0)
            {
                OUTPUTL(ne_scroll_forward, specified_window - vpos + 1);
            }
        }
        else
        {
            ...
            while (i-- != 0)
            {
                OUTPUTL(ne_scroll_reverse, specified_window - vpos + 1);
            }
        }

        if (specified_window != ne_lines)
        {
            set_scroll_region(0, ne_lines - 1);
        }
    }
    else
    {
...
void term_init(void)
{
    ...
    if (!ne_scroll_forward)
    {
        ne_scroll_forward = Down;
    }
    if (!ne_scroll_reverse)
    {
        ne_scroll_reverse = Up;
    }
