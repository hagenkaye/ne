...
char_pool *get_char_pool(buffer *const b, char *const p)
{
    ...
    {
        assert_char_pool(cp);
        if (p >= cp->pool && p < cp->pool + cp->size)
        {
            return cp;
        }
...
{
    if (pool_size < STD_LINE_DESC_POOL_SIZE)
    {
        pool_size = STD_LINE_DESC_POOL_SIZE;
    }
    ...
    if (ldp)
    {
        if ((ldp->pool = calloc(pool_size, do_syntax ? sizeof(line_desc) : sizeof(no_syntax_line_desc))))
        {
            ...
            for (int64_t i = 0; i < pool_size; i++)
                ...
                {
                    add_tail(&ldp->free_list, &((no_syntax_line_desc *)ldp->pool)[i].ld_node);
                }
            return ldp;
        }
        free(ldp);
...
int save_all_modified_buffers(void)
{
    ...
    for (buffer *b = (buffer *)buffers.head; b->b_node.next; b = (buffer *)b->b_node.next)
    {
        if (b->is_modified)
        {
            ...
            else if (save_buffer_to_file(b, NULL))
            {
                rc = ERROR;
            }
        }
...
int64_t alloc_chars_around(buffer *const b, line_desc *const ld, const int64_t n, const bool check_first_before)
{
    ...
    {
        if (cp->pool + cp->first_used == ld->line)
        {
            cp->first_used = (before + 1) - cp->pool;
        }
        if (cp->pool + cp->last_used == ld->line + ld->line_len - 1)
        {
            cp->last_used = (after - 1) - cp->pool;
        }
        b->free_chars -= n;
...
int delete_one_line(buffer *const b, line_desc *const ld, const int64_t line)
{
    ...
    if (ld->line_len && (b->last_deleted = reset_stream(b->last_deleted)))
    {
        add_to_stream(b->last_deleted, ld->line, ld->line_len);
    }
...
{
    if (!b || !ld || !stream || stream_len < 1 || pos > ld->line_len)
    {
        return ERROR;
    }
        ...
        if (len)
        {
            ...
            else
            {
                ...
                else   /* Third case. There are enough free characters around ld->line. */
                {
                    if (len - result)
                    {
                        memmove(ld->line - (len - result), ld->line, pos);
                    }
                    if (result)
                    {
                        memmove(ld->line + pos + result, ld->line + pos, ld->line_len - pos);
                    }
            ...
            {
                b->block_start_pos += len;
            }

            for (int i = 0, mask = b->bookmark_mask; mask; i++, mask >>= 1)
                if ((mask & 1) && b->bookmark[i].line == line && b->bookmark[i].pos > pos)
                {
                    b->bookmark[i].pos += len;
                }
        }
            ...
            if ((new_ld = alloc_line_desc(b)))
            {
                ...
                for (int i = 0, mask = b->bookmark_mask; mask; i++, mask >>= 1)
                {
                    if (mask & 1)
                    {
                        ...
                        else if (b->bookmark[i].line > line)
                        {
                            b->bookmark[i].line++;
                        }
                    }
    ...
    while (len)
    {
        ...
        if (pos == ld->line_len)
        {
            ...
            else
            {
                ...
                {
                    if (n < next_ld->line_len)
                    {
                        memmove(ld->line + (n - next_ld->line_len), ld->line, ld->line_len);
                    }
                ...
                else
                {
                    if (m)
                    {
                        memmove(next_ld->line + m, next_ld->line, next_ld->line_len);
                    }
                    next_ld->line += m;
            ...
            if (!b->redoing)
            {
                if (b->undoing)
                {
                    add_to_stream(&b->undo.redo, "", 1);
                }
                else if (b->opt.do_undo)
                {
                    add_to_undo_stream(&b->undo, "", 1);
                }
            }
        }
        ...
        else
        {
            ...
            if (!b->redoing)
            {
                if (b->undoing)
                {
                    add_to_stream(&b->undo.redo, &ld->line[pos], n);
                }
                else if (b->opt.do_undo)
                {
                    add_to_undo_stream(&b->undo, &ld->line[pos], n);
                }
            }

            if (n == ld->line_len - pos)
            {
                free_chars(b, &ld->line[pos], n);
            }
            else
            {
...
line_desc *nth_line_desc(const buffer *b, const int64_t n)
{
    ...
    if (best_absolute_cost < relative_cost)
    {
        ...
        else
        {
            ...
            for (int64_t i = 0; i < b->num_lines - 1 - n; i++)
            {
                if (i == -1)
                {
                    fputc('.', stderr);    // This is a nop that's here just to avoid a gcc bug
                }
    ...
    else
    {
        ...
        if (n < b->cur_line) for (int64_t i = 0; i < b->cur_line - n; i++)
            {
                ld = (line_desc *)ld->ld_node.prev;
            }
        else for (int64_t i = 0; i < n - b->cur_line; i++)
            {
                ld = (line_desc *)ld->ld_node.next;
            }
    }
...
int load_file_in_buffer(buffer *const b, const char *name)
{
    ...
    if (fh >= 0)
    {
        ...
        if (!result)
        {
            b->opt.read_only = (access(name, W_OK) != 0);
        }
        return result;
    }
...
int load_fh_in_buffer(buffer *b, int fh)
{
    ...
    {
        if (b->opt.utf8auto && encoding == ENC_UTF8)
        {
            b->encoding = ENC_UTF8;
        }
        else
        {
            b->encoding = ENC_8_BIT;
        }
    }
...
int save_buffer_to_file(buffer *b, const char *name)
{
    ...
    if (fh >= 0)
    {
        ...
        if (p)
        {
            ...
            if (!error && used && write(fh, p, used) < used)
            {
                error = IO_ERROR;
            }
