...
int dump_config(void)
{
    ...
    if (!(f = fopen("ne_test_dump_config", "w")))
    {
        return ERROR;
    }
...
static void draw_menu(const int n)
{
    ...
    if (menus[n].cur_item + 1 + (standout_ok == 0) >= ne_lines - 1)
    {
        menus[n].cur_item = 0;
    }
    ...
    {
        if (i + 1 + (standout_ok == 0) >= ne_lines - 1)
        {
            break;
        }
    ...
    {
        move_cursor(i + 1, menus[n].xpos - 1);
        for (i = 0; i < menus[n].width + (standout_ok ? MENU_EXTRA : MENU_NOSTANDOUT_EXTRA); i++)
        {
            output_string("-", false);
        }
    }
...
static void undraw_menu(const int n)
{
    ...
    for (int i = 1; i <= menus[n].item_num + (standout_ok == 0); i++)
    {
        ...
        if (ld->ld_node.next->next)
        {
            ...
            if (cur_buffer->syn)
            {
                parse(cur_buffer->syn, ld, ld->highlight_state, cur_buffer->encoding == ENC_UTF8);
            }
...
static void draw_next_item(void)
{
    ...
    if (menus[current_menu].cur_item + 1 + (standout_ok == 0) >= ne_lines - 1)
    {
        menus[current_menu].cur_item = 0;
    }
...
{
    undraw_cur_item(current_menu);
    if (--(menus[current_menu].cur_item) < 0)
    {
        menus[current_menu].cur_item = menus[current_menu].item_num - 1;
    }
    if (menus[current_menu].cur_item + 1 + (standout_ok == 0) >= ne_lines - 1)
    {
        menus[current_menu].cur_item = ne_lines - 3 - (standout_ok == 0) ;
    }
    draw_cur_item(current_menu);
}
...
static void draw_next_menu(void)
{
    ...
    if (menus[current_menu].xpos >= ne_columns)
    {
        current_menu = 0;
    }
...
static void draw_prev_menu(void)
{
    ...
    while (menus[current_menu].xpos >= ne_columns)
    {
        current_menu--;
    }
    draw_menu(current_menu);
...
int search_menu_title(int n, const int c)
{
    ...
    {
        if (menus[++n % menu_num].xpos >= ne_columns)
        {
            continue;
        }
        if (menus[n % menu_num].text[0] == c)
        {
            return n % menu_num;
        }
    }
...
int search_menu_item(int n, int c)
{
    ...
    {
        if (++j % menus[n].item_num + 1 + (standout_ok == 0) >= ne_lines - 1)
        {
            continue;
        }
        if (menus[n].items[j % menus[n].item_num].text[0] == c)
        {
            return j % menus[n].item_num;
        }
    }
...
void draw_status_bar(void)
{
    ...
    if (status_bar)
    {
        ...
        if (len < ne_columns - 1)
        {
            ...
            {
                output_string(UNNAMED_NAME, false);
            }
        }
        ...
        if (menu_conf)
        {
            ...
            if ((cs = load_stream(NULL, menu_conf_name, false, false)) || (cs = load_stream(NULL, menu_conf, false, false)))
            {
                for (int pass = 0; pass < 2; pass++)
                {
                    ...
                    while (p - cs->stream < cs->len)
                    {
                        if (*p)
                        {
                                        ...
                                        if (*p)
                                        {
                                            ...
                                            if (*p)
                                            {
                                                *p++ = 0;
                                                ...
                                                {
                                                    new_menus[cur_menu].xpos = new_menus[cur_menu - 1].xpos + strlen(new_menus[cur_menu - 1].text) + 1;
                                                }
                                                new_menus[cur_menu].items = &new_items[cur_item];
                                            }
                                            else
                                            {
                                                error_in_menu_configuration(line, "menu name has to end with quotes.");
                                            }
                                        }
                                        else
                                        {
                                            error_in_menu_configuration(line, "menu name has to start with quotes.");
                                        }
                                    }
                                }
                                else if (cur_menu >= 0)
                            ...
                            else if (!cmdcmp(ITEM_KEYWORD, p))
                            {
                                if (cur_menu < 0)
                                {
                                    error_in_menu_configuration(line, "no menu specified for this item.");
                                }
                                        ...
                                        if (*p)
                                        {
                                            *p++ = 0;
                                            if (num_items_in_menu == 0 || strlen(new_items[cur_item].text) == new_menus[cur_menu].width)
                                            {
                                                ...
                                                {
                                                    error_in_menu_configuration(line, "no command specified.");
                                                }
                                            }
                                            else
                                            {
                                                error_in_menu_configuration(line, "menu item name width has to be constant throughout the menu.");
                                            }
                                        }
                                        else
                                        {
                                            error_in_menu_configuration(line, "menu item name has to end with quotes.");
                                        }
                                    }
                                    else
                                    {
                                        error_in_menu_configuration(line, "menu item name has to start with quotes.");
                                    }
                                }
                                num_items_in_menu++;
                                cur_item++;
                    ...
                    if (pass == 0)
                    {
                        if (!num_items_in_menu)
                        {
                            error_in_menu_configuration(line - 1, "no items specified for this menu.");
                        }
                        if (cur_menu == -1 || cur_item == 0)
                        {
                            error_in_menu_configuration(line, "no menus or items specified.");
                        }
...
void get_menu_configuration(const char *menu_conf_name)
{
    ...
    if (menus == def_menus)
    {
        get_menu_conf(menu_conf_name, exists_gprefs_dir);
    }
}
...
{
    if (!key_bindings_name)
    {
        key_bindings_name = KEY_BINDINGS_NAME;
    }
            ...
            if (cs)
            {
                ...
                while (p - cs->stream < cs->len)
                {
                        ...
                        if (sscanf(p, "%x %*s", &c) == 1)
                        {
                            if (c >= 0 && c < NUM_KEYS)
                            {
                                if (c != 27 && c != 13)
                                {
                                    ...
                                    {
                                        error_in_key_bindings(line, "no command specified.");
                                    }
                                }
                                else
                                {
                                    error_in_key_bindings(line, "you cannot redefine ESCAPE and RETURN.");
                                }
                            }
                            else
                            {
                                error_in_key_bindings(line, "key code out of range.");
                            }
                        }
                        else
                        {
                            error_in_key_bindings(line, "can't read key code.");
                        }
                    }
                    else if (*p && !cmdcmp(SEQ_KEYWORD, p))
                    {
                        ...
                        while (isasciispace(*p))
                        {
                            p++;    /* skip to quoted sequence, like  "\x1b[A" */
                        }
                        ...
                        {
                            error_in_key_bindings(line, "can't read double quoted character sequence.");
                        }
                    }
                    line++;
                    p += strlen(p) + 1;
