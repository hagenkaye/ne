...
int do_action(buffer *b, action a, int64_t c, char *p)
{
    ...
    if (b->recording)
    {
        record_action(b->cur_macro, a, c, p, verbose_macros);
    }
    ...
    switch (a)
    {
        ...
        case SAVEALL_A:
            ...
            {
                print_message(info_msg[MODIFIED_SAVED]);
            }
            return OK;
        ...
        case QUIT_A:
            if (modified_buffers() && !request_response(b, info_msg[SOME_DOCUMENTS_ARE_NOT_SAVED], false))
            {
                return ERROR;
            }
            close_history();
        ...
        case GOTOBOOKMARK_A:
        {
            ...
            if (p)
            {
                if ((p[0] == '+' || p[0] == '-') && p[1] == '1')
                {
                    if (b->cur_bookmark < 0 || b->cur_bookmark > MAX_USER_BOOKMARK)
                    {
                        b->cur_bookmark = 0;
                    }
                    int i;
        ...
        case GOTOLINE_A:
            if (c < 0 && (c = request_number("Line", b->cur_line + 1)) < 0)
            {
                return NUMERIC_ERROR(c);
            }
            if (c == 0 || c > b->num_lines)
            {
                c = b->num_lines;
            }
            goto_line(b, --c);
        ...
        case GOTOCOLUMN_A:
            if (c < 0 && (c = request_number("Column", b->cur_x + b->win_x + 1)) < 0)
            {
                return NUMERIC_ERROR(c);
            }
            goto_column(b, c ? --c : 0);
        ...
        case INSERTSTRING_A:
            ...
            if (p || (p = request_string("String", NULL, false, COMPLETE_NONE, b->encoding == ENC_UTF8 || (b->encoding == ENC_ASCII && b->opt.utf8auto))))
            {
                ...
                if (b->encoding == ENC_ASCII || encoding == ENC_ASCII || (b->encoding == encoding))
                {
                    ...
                    for (int64_t pos = 0; p[pos] && error == OK; pos = next_pos(p, pos, encoding))
                    {
                        error = do_action(b, INSERTCHAR_A, get_char(&p[pos], encoding), NULL);
                    }
                }
                else
                {
        ...
        case INSERTCHAR_A:
        {
            ...
            if (c < 0 && (c = request_number("Char Code", last_inserted_char)) < 0)
            {
                return NUMERIC_ERROR(c);
            }
            if (c == 0)
            {
                return CANT_INSERT_0;
            }
            ...
            if (b->encoding == ENC_ASCII)
            {
                ...
                {
                    b->encoding = b->opt.utf8auto ? ENC_UTF8 : ENC_8_BIT;
                }
            }
            if (c > 0xFF && b->encoding == ENC_8_BIT)
            {
            ...
            if (b->cur_pos > b->cur_line_desc->line_len)
            {
                ...
                if (b->syn)
                {
                    update_line(b, b->cur_y, true, true);
                }
            }
            ...
            if (!deleted_char)
            {
                update_inserted_char(b, c, b->cur_line_desc, b->cur_pos, b->cur_char, b->cur_y, b->cur_x);
            }
            else
            {
                update_overwritten_char(b, old_char, c, b->cur_line_desc, b->cur_pos, b->cur_char, b->cur_y, b->cur_x);
            }
            ...
            if (error == ERROR)
            {
                ...
                if (b->syn)
                {
                    update_line(b, b->cur_y, true, false);
                }
                assert_buffer_content(b);
            }
            else
            {
                ...
                {
                    update_partial_line(b, b->cur_y, calc_width(b->cur_line_desc, b->cur_line_desc->line_len, b->opt.tab_size, b->encoding) - b->win_x, false, false);
                }
        ...
        case DELETECHAR_A:
            ...
            for (int64_t i = 0; i < c && !stop; i++)
            {
                if (a == BACKSPACE_A)
                {
                    ...
                    else
                    {
                            ...
                            && (b->cur_pos > b->cur_line_desc->line_len || b->cur_line_desc->line[b->cur_pos - 1] == ' '))
                        {
                            ...
                            while ((b->win_x + b->cur_x) % b->opt.tab_size != 0
                                   && (b->cur_pos > b->cur_line_desc->line_len || b->cur_line_desc->line[b->cur_pos - 1] == ' '));
                        }
                        else
                        {
                    ...
                    ((b->win_x + b->cur_x) % b->opt.tab_size == 0 || b->cur_line_desc->line[b->cur_pos - 1] != ' '))
                {
                    ...
                    while ((b->win_x + b->cur_x + col) % b->opt.tab_size != 0
                    ...
                    if (col > 1 && (b->win_x + b->cur_x + col) % b->opt.tab_size == 0)
                    {
                        ...
                        if (b->syn)
                        {
                            update_partial_line(b, b->cur_y, b->cur_x, true, true);
                        }
                    }
                }
                ...
                if (b->cur_pos > b->cur_line_desc->line_len)
                {
                    ...
                    if (b->cur_line_desc->ld_node.next->next == NULL)
                    {
                        continue;
                    }
                ...
                else
                {
                    ...
                    if (b->syn && b->cur_pos == 0)
                    {
                        next_line_state = b->cur_line_desc->highlight_state;
                    }
                    delete_one_char(b, b->cur_line_desc, b->cur_line, b->cur_pos);
                    if (b->syn && b->cur_pos == 0)
                    {
                        b->cur_line_desc->highlight_state = next_line_state;
                    }
                    ...
                    {
                        update_partial_line(b, b->cur_y, b->cur_x, true, false);
                    }

                    if (b->cur_y < ne_lines - 2)
                    {
                        scroll_window(b, b->cur_y + 1, -1);
                    }
                }
            }
            need_attr_update = true;
            ...
            for (int64_t i = 0; i < c && !stop; i++)
            {
                ...
                if (insert_one_line(b, b->cur_line_desc, b->cur_line, b->cur_pos > b->cur_line_desc->line_len ? b->cur_line_desc->line_len : b->cur_pos) == OK)
                {
                    ...
                    else
                    {
                        ...
                        {
                            update_partial_line(b, b->cur_y, b->cur_x, false, false);
                        }
        ...
        case SAVEAS_A:
            if (p || (q = p = request_file(b, "Filename", b->filename)))
            {
                ...
                if (a == SAVE_A && buffer_file_modified(b, p) && !request_response(b, info_msg[FILE_HAS_BEEN_MODIFIED], false))
                {
                ...
                if (!dup || dup == b || (dprompt && !c) || (dprompt = request_response(b, info_msg[SAME_NAME], false)))
                {
                        ...
                        && error != OUT_OF_MEMORY)
                    {
                        ...
                        if (b->opt.auto_prefs && extension(p))
                        {
                            ...
                            {
                                error = FILE_TOO_LARGE_SYNTAX_HIGHLIGHTING_DISABLED;
                            }
                        }
                    }
                    print_error(error);
                ...
                if (p || (p = request_string(b->last_was_regexp ? "Replace RegExp" : "Replace", b->replace_string, true, COMPLETE_NONE, b->encoding == ENC_UTF8 || (b->encoding == ENC_ASCII && b->opt.utf8auto))))
                {
                           ...
                           !(error = (b->last_was_regexp ? find_regexp : find)(b, NULL, !first_search && a != REPLACEALL_A && c != 'A' && c != 'Y')))
                    {
                        ...
                        if (c == 'A' || c == 'Y' || c == 'L' || a == REPLACEONCE_A || a == REPLACEALL_A)
                        {
                            ...
                            if (b->last_was_regexp)
                            {
                                error = replace_regexp(b, p);
                            }
                            else
                            {
                                error = replace(b, strlen(b->find_string), p);
                            }
        ...
        case REPEATLAST_A:
            ...
            else if ((b->last_was_replace) && !b->replace_string)
            {
                return NO_REPLACE_STRING;
            }
            else
            {
                int return_code = 0;
                ...
                for (int64_t i = 0; i < c; i++)
                {
                    if (!print_error((b->last_was_regexp ? find_regexp : find)(b, NULL, !b->last_was_replace)))
                    {
                        if (b->last_was_replace)
                        {
                            if (b->last_was_regexp)
                            {
                                error = replace_regexp(b, b->replace_string);
                            }
                            else
                            {
                                error = replace(b, strlen(b->find_string), b->replace_string);
                            }
        ...
        case ESCAPETIME_A:
            if (c < 0 && (c = request_number("Timeout (1/10s)", -1)) < 0)
            {
                return NUMERIC_ERROR(c);
            }
            if (c < 256)
            ...
            {
                return ESCAPE_TIME_OUT_OF_RANGE;
            }

        case TABSIZE_A:
            if (c < 0 && (c = request_number("TAB Size", b->opt.tab_size)) <= 0)
            {
                return NUMERIC_ERROR(c);
            }
            if (c < ne_columns / 2)
        ...
        case TURBO_A:
            if (c < 0 && (c = request_number("Turbo Threshold", turbo)) < 0)
            {
                return NUMERIC_ERROR(c);
            }
            turbo = c;
        ...
        case CLIPNUMBER_A:
            if (c < 0 && (c = request_number("Clip Number", b->opt.cur_clip)) < 0)
            {
                return NUMERIC_ERROR(c);
            }
            b->opt.cur_clip = c;
        ...
        case RIGHTMARGIN_A:
            if (c < 0 && (c = request_number("Right Margin", b->opt.right_margin)) < 0)
            {
                return NUMERIC_ERROR(c);
            }
        ...
        case UTF8_A:
        {
            ...
            if ((c < 0 && b->encoding != ENC_UTF8) || c > 0)
            {
                ...
                {
                    return BUFFER_IS_NOT_UTF8;
                }
            }
            else
            {
                b->encoding = encoding == ENC_ASCII ? ENC_ASCII : ENC_8_BIT;
            }
        ...
        case PLAY_A:
            ...
            {
                if (c < 0 && (c = request_number("Times", 1)) <= 0)
                {
                    return NUMERIC_ERROR(c);
                }
        ...
        case SAVEMACRO_A:
            if (p || (p = request_file(b, "Macro Name", NULL)))
            {
                ...
                if ((error = print_error(save_stream(b->cur_macro, p, b->is_CRLF, false))) == OK)
                {
                    print_info(SAVED);
                }
                free(p);
        ...
        case NEXTDOC_A: /* Was NEXT_BUFFER: */
            if (b->b_node.next->next)
            {
                cur_buffer = (buffer *)b->b_node.next;
            }
            else
            {
                cur_buffer = (buffer *)buffers.head;
            }
            keep_cursor_on_screen(cur_buffer);
            reset_window();
            need_attr_update = false;
        ...
        case PREVDOC_A:
            if (b->b_node.prev->prev)
            {
                cur_buffer = (buffer *)b->b_node.prev;
            }
            else
            {
                cur_buffer = (buffer *)buffers.tail_pred;
            }
            keep_cursor_on_screen(cur_buffer);
            reset_window();
            need_attr_update = false;
        ...
        case SAVECLIP_A:
            ...
            {
                print_info(SAVING);
                if ((error = print_error(save_clip(b->opt.cur_clip, p, b->is_CRLF, b->opt.binary))) == OK)
                {
                    print_info(SAVED);
                }
                free(p);
        ...
        case THROUGH_A:
            ...
            if (p || (p = request_string("Filter", NULL, false, COMPLETE_FILE, b->encoding == ENC_UTF8 || (b->encoding == ENC_ASCII && b->opt.utf8auto))))
            {
                ...
                if ((fin = mkstemp(tmpnam1)) != -1)
                {
                    close(fin);
                }
                if ((fout = mkstemp(tmpnam2)) != -1)
                {
                    close(fout);
                }
                if (fin != -1 && fout != -1)
                            ...
                            if ((command = malloc(strlen(p) + strlen(tmpnam1) + strlen(tmpnam2) + 16)))
                            {
                                ...
                                if (!error)
                                {
                                    if (!(error = load_clip(INT_MAX, tmpnam2, b->opt.preserve_cr, b->opt.binary)))
                                    {
                                        ...
                                        if (b->marking)
                                        {
                                            (b->mark_is_vertical ? erase_vert_block : erase_block)(b);
                                        }
                ...
                {
                    error = CANT_OPEN_TEMPORARY_FILE;
                }
        ...
        case SYNTAX_A:
            ...
            if (p || (p = request_string("Syntax",  b->syn ? (const char *)b->syn->name : NULL, true, COMPLETE_SYNTAX, b->encoding == ENC_UTF8 || (b->encoding == ENC_ASCII && b->opt.utf8auto))))
            {
                ...
                {
                    error = print_error(load_syntax_by_name(b, p));
                }
                reset_window();
                free(p);
                return error ? ERROR : OK;
        ...
        case AUTOCOMPLETE_A:
            ...
            if ((p = autocomplete(p, msg, true, &e)))
            {
                b->recording = 0;
                start_undo_chain(b);
                if (pos >= b->cur_pos || (error = do_action(b, DELETEPREVWORD_A, 1, NULL)) == OK)
                {
            ...
            else if (e == AUTOCOMPLETE_NO_MATCH)
            {
                print_message(info_msg[AUTOCOMPLETE_NO_MATCH]);
            }
