...
void resync_pos(buffer *const b)
{
    ...
    for (i = pos = width = 0; pos < ld->line_len; pos = next_pos(ld->line, pos, b->encoding), i++)
    {
        ...
        if (ld->line[pos] != '\t')
        {
            width += (last_char_width = get_char_width(&ld->line[pos], b->encoding));
        }
        else
        {
            width += (last_char_width = b->opt.tab_size - width % b->opt.tab_size);
        }
        ...
        if (width > x)
        {
            ...
            else if (width - b->win_x < ne_columns)
            {
                b->cur_x = width - b->win_x;
            }
            else
...
void goto_line(buffer *const b, const int64_t n)
{
    ...
    if (b->win_y > b->num_lines - (ne_lines - 1))
    {
        b->win_y = b->num_lines - (ne_lines - 1);
    }
    if (b->win_y < 0)
    {
        b->win_y = 0;
...
void move_to_eol(buffer *const b)
{
    ...
    for (int64_t i = 0, pos = 0, width = 0; pos < ld->line_len; pos = next_pos(ld->line, pos, b->encoding), i++)
    {
        if (ld->line[pos] != '\t')
        {
            width += get_char_width(&ld->line[pos], b->encoding);
        }
        else
        {
            width += b->opt.tab_size - width % b->opt.tab_size;
        }
...
void move_to_bof(buffer *const b)
{
    ...
    for (int i = 0; i < ne_lines - 2 && ld->ld_node.prev->prev; i++)
    {
        ld = (line_desc *)ld->ld_node.prev;
    }
...
int search_word(buffer *const b, const int dir)
{
    ...
    else if (!ne_isword(get_char(&ld->line[pos], b->encoding), b->encoding))
    {
        space_skipped = true;
    }
    ...
    while (y < b->num_lines && y >= 0)
    {
        ...
        else
        {
            ...
            if (ld->ld_node.prev)
            {
                pos = prev_pos(ld->line, ld->line_len, b->encoding);
            }
        }
    }
    return ERROR;
...
void move_to_eow(buffer *const b)
{
    ...
    line_desc *ld = b->cur_line_desc;
    int64_t pos = b->cur_pos;
    if (pos >= ld->line_len || !ne_isword(get_char(&ld->line[pos], b->encoding), b->encoding))
    {
        return;
    }

    for (pos = b->cur_pos; pos < ld->line_len; pos += b->encoding == ENC_UTF8 ? utf8len(ld->line[pos]) : 1)
        if (!ne_isword(get_char(&ld->line[pos], b->encoding), b->encoding))
        {
            break;
        }
