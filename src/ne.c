...
int main(int argc, char **argv)
{
    ...
    if (!skiplist)
    {
        exit(1);    /* We need this many flags. */
    }
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        if (argv[i][0] == '-' && argv[i][1] == '-')
        {
            if (!argv[i][2])
            {
                i++;    /* You can use "--" to force the next token to be a filename */
            }
    ...
    if (argc > 1)
    {
            ...
            if (argv[i] && !skiplist[i])
            {
                ...
                else
                {
                    if (!strcmp(argv[i], "-") && stdin_buffer)
                    {
                        ...
                        if (read_only)
                        {
                            stdin_buffer->opt.read_only = read_only;
                        }
                        if (first_line)
                        {
                            do_action(stdin_buffer, GOTOLINE_A, first_line, NULL);
                        }
                        if (first_col)
                        {
                            do_action(stdin_buffer, GOTOCOLUMN_A, first_col, NULL);
                        }
                        stdin_buffer = NULL;
                    }
                    else
                    {
                        ...
                        if (!first_file)
                        {
                            do_action(cur_buffer, NEWDOC_A, -1, NULL);
                        }
                        else
                        {
                            first_file = false;
                        ...
                        if (first_line)
                        {
                            do_action(cur_buffer, GOTOLINE_A, first_line, NULL);
                        }
                        if (first_col)
                        {
                            do_action(cur_buffer, GOTOCOLUMN_A, first_col, NULL);
                        }
                        if (read_only)
                        {
                            cur_buffer->opt.read_only = read_only;
                        }
                    }
                    first_line =
                        first_col  = 0;
        ...
        if (get_nth_buffer(1))
        {
            do_action(cur_buffer, NEXTDOC_A, -1, NULL);
        }
    ...
    if (macro_name)
    {
        do_action(cur_buffer, MACRO_A, -1, str_dup(macro_name));
    }
    else if (first_file)
    {
    ...
    while (true)
    {
        ...
        {
            automatch_bracket(cur_buffer, false);
        }
