...
static void search_buff(const buffer *b, char *p, const int encoding, const bool case_search, const int ext)
{
    ...
    for (line_desc *ld = (line_desc *)b->line_desc_list.head, *next; (next = (line_desc *)ld->ld_node.next); ld = next)
    {
        ...
        do
        {
            ...
            while (l < ld->line_len - p_len && !ne_isword(get_char(&ld->line[l], b->encoding), b->encoding))
            {
                l += get_char_width(&ld->line[l], b->encoding);
            }
...
char *autocomplete(char *p, char *req_msg, const int ext, int *const error)
{
    ...
    if (rl.cur_entries)
    {
        if (rl.entries[rl.cur_entries / 2][strlen(rl.entries[rl.cur_entries / 2]) - 1] == EXTERNAL_FLAG_CHAR)
        {
            rl.entries[rl.cur_entries / 2][strlen(rl.entries[rl.cur_entries / 2]) - 1] = 0;
        }
    ...
    *error = AUTOCOMPLETE_COMPLETED;
    ...
    if (rl.cur_entries > 0)
    {
        ...
        if (rl.entries[0][m - 1] == EXTERNAL_FLAG_CHAR)
        {
            m--;
        }
        for (int i = 1; i < rl.cur_entries; i++)
        {
            int j;
            for (j = 0; j < m; j++)
                if (rl.entries[i][j] != rl.entries[0][j])
                {
                    break;
                }
            m = j;
        ...
        else
        {
            ...
            if (result != ERROR)
            {
                ...
                if (rl.entries[result][strlen(rl.entries[result]) - 1] == EXTERNAL_FLAG_CHAR)
                {
                    rl.entries[result][strlen(rl.entries[result]) - 1] = 0;
                }
