...
int save_prefs(buffer *const b, const char *const name)
{
    ...
    if (cs)
    {
        ...
        if (!saving_global && b->syn)
        {
            record_action(cs, SYNTAX_A, -1, (const char *)b->syn->name, verbose_macros);
        }
        ...
        if (saving_global)
        {
            ...
            if (req_order)
            {
                record_action(cs, REQUESTORDER_A,  req_order,      NULL, verbose_macros);
            }
#else
            if (!req_order)
            {
            ...
            if (fast_gui)
            {
                record_action(cs, FASTGUI_A,       fast_gui,       NULL, verbose_macros);
            }
            if (!status_bar)
            {
                record_action(cs, STATUSBAR_A,     status_bar,     NULL, verbose_macros);
            }
            if (!verbose_macros)
            {
...
int load_syntax_by_name(buffer *const b, const char *const name)
{
    ...
    if (!syn)
    {
        syn = load_syntax((unsigned char *)ext2syntax(name));
    }
    if (syn)
    {
        b->syn = syn;
