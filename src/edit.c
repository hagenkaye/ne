...
static int to_something(buffer *b, int (to_first)(int), int (to_rest)(int))
{
    ...
    if (b->cur_line == b->num_lines - 1 && b->cur_pos >= b->cur_line_desc->line_len)
    {
        return ERROR;
    }
    ...
    if (changed)
    {
        ...
        while (pos < b->cur_line_desc->line_len && ne_isword(c = get_char(&b->cur_line_desc->line[pos], b->encoding), b->encoding))
        {
            if (b->encoding == ENC_UTF8)
            {
                new_len += utf8str(new_len ? to_rest(c) : to_first(c), word + new_len);
            }
            else
            {
...
int find_matching_bracket(buffer *b, const int64_t min_line, int64_t max_line, int64_t *match_line, int64_t *match_pos, int *c, line_desc **match_ld)
{
    ...
    {
        for (j = 0; j < 2; j++)
            if (ld->line[b->cur_pos] == bracket_table[i][j])
            {
                break;
            }
        if (j < 2)
    ...
    while (ld->ld_node.next && ld->ld_node.prev && y >= min_line && y <= max_line)
    {
        ...
        if (pos >= 0)
        {
            ...
            while (pos >= 0 && pos < ld->line_len)
            {
                ...
                else if (line[pos] == bracket_table[i][1 - j])
                {
                    n--;
                }
                ...
                {
                    pos = prev_pos(line, pos, b->encoding);
                }
            }
        }
...
int word_wrap(buffer *const b)
{
    ...
    while (first_pos < len && ne_isspace(get_char(&line[first_pos], b->encoding), b->encoding))
    {
        first_pos = next_pos(line, first_pos, b->encoding);
    }
...
static int save_spots(line_desc *const ld, const int pos, const encoding_type encoding)
{
    ...
    if (!ld->line || ld->line_len <= pos)
    {
        return 0;    /* No data on this line. */
    }
    ...
    if (pa_spots_pos)
    {
        if ((pa_spots = malloc(pa_spots_pos)))
        {
            memcpy(pa_spots, ld->line + pos, pa_spots_pos);
        }
        else
        {
            pa_spots_pos = 0;
...
static int save_space(line_desc *const ld, const int tab_size, const encoding_type encoding)
{
    ...
    if (pos == ld->line_len)
    {
        return 0;    /* Blank lines don't count. */
    }
...
static void trim_trailing_space(buffer *const b, line_desc *ld, const int64_t line, const encoding_type encoding)
{
    ...
    while (pos > 0 && isasciispace(ld->line[pos - 1]))
    {
        pos = prev_pos(ld->line, pos, encoding);
    }
    if (pos >= 0 && pos < ld->line_len)
    {
...
int center(buffer *const b)
{
    ...
    {
        start_pos = next_pos(ld->line, start_pos, b->encoding);
    }
    if (start_pos == ld->line_len)
    {
        return OK;
    }
    while (isasciispace(ld->line[prev_pos(ld->line, end_pos, b->encoding)]))
    {
        end_pos = prev_pos(ld->line, end_pos, b->encoding);
    }
...
int auto_indent_line(buffer *const b, const int64_t line, line_desc *const ld, const int64_t up_to_col)
{
    ...
    if (!prev_ld->ld_node.prev || prev_ld->line_len == 0)
    {
        return 0;
    }
...
int shift(buffer *const b, char *p, char *msg, int msg_size)
{
    ...
    if (!rc)
    {
        ...
        for (int64_t line = first_line; line <= last_line; line++)
        {
            ...
            if (pos >= ld->line_len)
            {
                continue;    /* We ran off the end of the line. */
            }
            ...
            while (!stop && (offset = calc_width(ld, c_pos, b->opt.tab_size, b->encoding) - c_col_orig - shift_size))
            {
                if (offset > 0)   /* still too far right; remove whitespace */
                {
                    ...
                    else if (pos)   /* should be a tab just to our left */
                    {
                        ...
                        {
                            break;    /* Should have been a tab. This should never happen! Give up on this line and go mangle the next one. */
                        }
                    }
                    else
                    {
                        break;    /* This should never happen; give up on this line and go mangle the next one. */
                    }
                }
