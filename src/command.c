...
static int hash_cmd(const char *const s, int len)
{
    ...
    while (len-- != 0)
    {
        h = (h * 31 + ascii_up_case[(unsigned char)s[len]]) % HASH_TABLE_SIZE;
    }
...
static int hash_macro(const char *const s, int len)
{
    ...
    while (len-- != 0)
    {
        h = (h * 31 + ascii_up_case[(unsigned char)s[len]]) % MACRO_HASH_TABLE_SIZE;
    }
...
int parse_command_line(const char *command_line, int64_t *const num_arg, char **const string_arg, const bool exec_only_options)
{
    ...
    while (isasciispace(*command_line))
    {
        command_line++;
    }
                ...
                if ((commands[a].flags & IS_OPTION) || !exec_only_options)
                {
                    if (*p)
                    {
                        if ((commands[a].flags & ARG_IS_STRING) && string_arg)
                        {
                            ...
                            if (len == 0 && !(commands[a].flags & EMPTY_STRING_OK))
                            {
...
int execute_command_line(buffer *b, const char *command_line)
{
    ...
    {
        return INCOMPATIBLE_COMMAND_ENCODING;
    }

    int64_t n;
    int a;
    char *p;
    if ((a = parse_command_line(command_line, &n, &p, b->exec_only_options)) >= 0)
    {
        return do_action(b, a, n, p);
    }
    a = -a;
...
{
    if (commands[a].flags & DO_NOT_RECORD)
    {
        return;
    }
    ...
    if (verbose)
    {
        add_to_stream(cs, commands[a].name, strlen(commands[a].name));
    }
    else
    {
        add_to_stream(cs, commands[a].short_name, strlen(commands[a].short_name));
    }
...
bool vet_optimize_macro_stream(char_stream *const cs, int64_t pos)
{
    ...
    while (pos < cs->len)
    {
        if ((a = parse_command_line(&cs->stream[pos], &n, &p, 0)) >= 0)
        {
            ...
            if (a == UNDO_A)
            {
                return false;    /* optimization is not safe */
            }
        }
        else
        {
            a = -a;
            if (a == NO_SUCH_COMMAND)
            {
                return false;    /* possibly a macro invocation */
            }
        }
...
void help(char *p)
{
    ...
    do
    {
        ...
        if (p || (r = request_strings(&rl, r)) >= 0)
        {
            ...
            if (p)
            {
                for (r = 0; r < strlen(p); r++) if (isasciispace((unsigned char)p[r]))
                    {
                        break;
                    }
