...
int find(buffer *const b, const char *pattern, const bool skip_first)
{
    ...
    if (! b->opt.search_back)
    {
        if (recompile_string)
        {
            for (int i = 0; i < m - 1; i++)
            {
                d[CONV((unsigned char)pattern[i])] = m - i - 1;
            }
    ...
    else
    {
        if (recompile_string)
        {
            for (int i = m - 1; i > 0; i--)
            {
                d[CONV((unsigned char)pattern[i])] = i;
            }
            b->find_string_changed = 0;
        }
...
int replace(buffer *const b, const int n, const char *const string)
{
    ...
    if (len)
    {
        insert_stream(b, b->cur_line_desc, b->cur_line, b->cur_pos, string, len);
    }
            ...
            do
            {
                if (!escape)
                {
                    ...
                    else if (*s == '[')
                    {
                        ...
                        do if (utf8len(*(++s)) != 1)
                            {
                                return UTF8_REGEXP_CHARACTER_CLASS_NOT_SUPPORTED;
                            } while (*s && *s != ']');
                    }
                    else if (*s == '\\')
                    {
