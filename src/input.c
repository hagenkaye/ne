...
char request_char(const buffer *const b, const char *const prompt, const char default_value)
{
    ...
    if (default_value)
    {
        output_char(default_value, 0, false);
    }
    ...
    while (true)
    {
        ...
        while (c > 0xFF || (ic = CHAR_CLASS(c)) == IGNORE || ic == INVALID);
...
int64_t request_number(const char *const prompt, const int64_t default_value)
{
    ...
    if (default_value >= 0)
    {
        sprintf(t, "%" PRId64, default_value);
    }
...
static void input_refresh(void)
{
    ...
    {
        if (i + get_char_width(&input_buffer[j], encoding) >= ne_columns)
        {
            break;
        }
...
static void input_autocomplete(void)
{
    ...
    {
        p = malloc(1);    /* no prefix left of the cursor; we'll give an empty one. */
    }
    p[pos - prefix_pos] = 0;
    ...
    if (p = autocomplete(p, NULL, true, &ac_err))
    {
        ...
        else
        {
            ...
            int ac_len = strlen(p);
            if (ac_len + len >= MAX_INPUT_LINE_LEN)
            {
                ac_len = MAX_INPUT_LINE_LEN - len;
            }
...
static void input_paste(void)
{
    ...
    if (cd)
    {
        ...
        if (len + paste_len > MAX_INPUT_LINE_LEN)
        {
            paste_len = MAX_INPUT_LINE_LEN - len;
        }
        memmove(&input_buffer[pos + paste_len], &input_buffer[pos], len - pos + 1);
        strncpy(&input_buffer[pos], cd->cs->stream, paste_len);
        len += paste_len;
        if (!input_buffer_is_ascii() && cd->cs->encoding != ENC_ASCII)
        {
            encoding = cd->cs->encoding;
        }
        input_refresh();
    }
}
...
char *request(const char *prompt, const char *const default_string, const bool alpha_allowed, const int completion_type, const bool prefer_utf8)
{
    ...
    while (true)
    {
        ...
        if (ic == ALPHA && c > 0xFF && encoding != ENC_ASCII && encoding != ENC_UTF8)
        {
            ic = INVALID;
        }
        ...
        if (ic != TAB)
        {
            last_char_completion = false;
        }
        if (ic == TAB && !completion_type)
        {
            ic = ALPHA;
        }
        ...
        switch (ic)
        {
            ...
            case ALPHA:
                ...
                if (encoding == ENC_ASCII && c > 0x7F)
                {
                    encoding = prefer_utf8 || c > 0xFF ? ENC_UTF8 : ENC_8_BIT;
                }
                ...
                if (len <= MAX_INPUT_LINE_LEN - c_len && (alpha_allowed || (c < 0x100 && isxdigit(c)) || c == 'X' || c == 'x'))
                {
                    ...
                    if (x < ne_columns - c_width)
                    {
                        if (pos == len - c_len)
                        {
                            output_char(c, 0, encoding);
                        }
                        else if (char_ins_del_ok)
                        {
                            insert_char(c, 0, encoding);
                        }
                        else
                        {
                            input_refresh();
        ...
        if (selection)
        {
            ...
            assert(input_buffer[len] == 0);
            if (history_buff->num_lines == 0 || len != last->line_len || strncmp(input_buffer, last->line, last->line_len))
            {
