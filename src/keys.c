...
int binsearch(const char *const s)
{
    if (num_keys)
    {
        ...
        {
            m = (l + r) / 2;
            if (is_prefix(s, key[m].string))
            {
                return -m - 1;
            }
            if (strcmp(key[m].string, s) > 0)
            {
                l = m + 1;
            }
            else if (strcmp(key[m].string, s) < 0)
            {
                r = m - 1;
            }
        }
...
void read_key_capabilities(void)
{
    ...
    for (int i = 1; i < 128; i++)
    {
        key_may_set(meta_prefixed[i], NE_KEY_META(i));
    }
...
int get_key_code(void)
{
    ...
    while (true)
    {
            ...
            while (last_match < cur_len)
            {
                ...
                else if (partial_is_utf8)       /* Our partial match is an UTF-8 sequence. */
                {
                    if ((kbd_buffer[last_match] & 0xC0) == 0x80)
                    {
                        if (utf8len(kbd_buffer[0]) == ++last_match)
                        {
                            ...
                            if (cur_len -= last_match)
                            {
                                memmove(kbd_buffer, kbd_buffer + last_match, cur_len);
                            }
                    ...
                    else
                    {
                        ...
                        if (--cur_len)
                        {
                            memmove(kbd_buffer, kbd_buffer + 1, cur_len);
                        }
                ...
                else
                {
                    ...
                    else if (key[cur_key].string[last_match] == kbd_buffer[last_match])
                    {
                        ...
                        {
                            if (cur_len -= last_match + 1)
                            {
                                memmove(kbd_buffer, kbd_buffer + last_match + 1, cur_len);
                            }
        ...
        if (partial_match)
        {
            set_termios_timeout(escape_time);
        }
        ...
        if (c != EOF)
        {
            if (cur_len < KBD_BUF_SIZE)
            {
                kbd_buffer[cur_len++] = c;
            }
        }
        else
        {
            if (cur_len)
            {
                ...
                if (partial_is_utf8)
                {
                    cur_len = last_match = partial_is_utf8 = 0;
                }
                else
                {
                    c = kbd_buffer[0];
