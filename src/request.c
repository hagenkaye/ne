        ...
        if (row < NAMES_PER_COL(page))
        {
            for (int col = 0; col < NAMES_PER_LINE(page); col++)
            {
                if (PXY2N(page, col, row) < rl.cur_entries)
                {
                    ...
                    if (rl.suffix)
                    {
                        set_attr(p[strlen(p) - 1] == rl.suffix ? BOLD : 0);
                    }
...
{
    if (! rl0->allow_reorder || rl.cur_entries < 2)
    {
        return false;
    }
...
static void fuzz_back()
{
    ...
    if (prune)
    {
        if (orig_entries == rl0->cur_entries)
        {
            return;
        }
        ...
        if (shiftsize)
        {
            ...
            for (rldest = rl0src = 0, rlsrc = shiftsize; rl0src < rl0->cur_entries; rl0src++)
            {
                ...
                if (rl0->entries[rl0src] == rl.entries[rlsrc])
                {
                    ...
                    if (n0shifted == rlsrc)
                    {
                        n1 = rldest;    /* it was our marked entry */
                    }
...
int request_strings(req_list *rlp0, int n)
{
    ...
    while (true)
    {
        if (ne_lines0 != ne_lines || ne_columns0 != ne_columns)
        {
            ...
            if (!(max_names_per_line = ne_columns / dx))
            {
                max_names_per_line = 1;
            }
        ...
        while ((ic = CHAR_CLASS(c)) == IGNORE || ic == INVALID);
...
static void load_syntax_names(req_list *rl, DIR *d, int flag)
{
    ...
    for (struct dirent * de; !stop && (de = readdir(d));)
    {
        ...
        if (len > extlen && !strcmp(de->d_name + len - extlen, SYNTAX_EXT))
        {
            ...
            if (!req_list_add(rl, de->d_name, flag))
            {
                break;
            }
...
char *request_syntax()
{
    ...
    if (req_list_init(&rl, filenamecmp, false, false, '*') != OK)
    {
        return NULL;
    }
...
char *request_files(const char *const filename, bool use_prefix)
{
    ...
    {
        next_dir = false;
        if (req_list_init(&rl, filenamecmp, true, false, '/') != OK)
        {
            break;
        }
        ...
        if (d)
        {
            ...
            for (struct dirent * de; !stop && (de = readdir(d));)
            {
                ...
                if (use_prefix && !is_prefix(file_part(filename), de->d_name))
                {
                    continue;
                }
                if (!req_list_add(&rl, de->d_name, is_dir))
                {
                    break;
                }
            }
...
char *req_list_add(req_list *const rl, char *const str, const int suffix)
{
    ...
    if (rl->cmpfnc)   /* implies the entries are sorted */
    {
        ...
        {
            ins = m + 1; /* insert at i + 1 */
        }
        else
        {
            /* impossible! */ ins = rl->cur_entries;
        }
    }
    else  /* not ordered */
    {
